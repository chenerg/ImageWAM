...
from .datasets.image_writer import AsyncImageWriter, write_image
    ...
    def get_episode_data(self, episode_idx: int) -> dict:
        for dataset in self._datasets:
            if episode_idx < dataset.num_episodes:
                ...
                file = dataset.root / dataset.meta.get_data_file_path(episode_id)
                table = pq.read_table(str(file))
                if "episode_index" in table.column_names:
                    table = table.filter(pc.equal(table["episode_index"], episode_id))
...
MultiLeRobotDatasetV3 = MultiLeRobotDataset
