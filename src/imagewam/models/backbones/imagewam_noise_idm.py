...
class ImageWAMNoiseIDM(ImageWAMIDM):
    """IDM-style variant where actions condition on first frame + pure-noise video latents."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._configure_noise_idm_training(
            noise_idm_train_schedule=noise_idm_train_schedule,
            noise_idm_alternate_loss_scale=noise_idm_alternate_loss_scale,
            noise_idm_ddp_zero_anchor=noise_idm_ddp_zero_anchor,
        )
    ...
    def from_wan22_pretrained(
        ...
        **kwargs,
        ...
        model._configure_noise_idm_training(
            noise_idm_train_schedule=noise_idm_train_schedule,
            noise_idm_alternate_loss_scale=noise_idm_alternate_loss_scale,
            noise_idm_ddp_zero_anchor=noise_idm_ddp_zero_anchor,
        )
    ...
    def _configure_noise_idm_training(
        self,
        noise_idm_train_schedule: str,
        noise_idm_alternate_loss_scale: float,
        noise_idm_ddp_zero_anchor: bool,
    ) -> None:
        schedule = str(noise_idm_train_schedule).strip().lower()
        if schedule not in {"joint", "alternate_micro"}:
            raise ValueError(
                "`noise_idm_train_schedule` must be 'joint' or 'alternate_micro', "
                f"got {noise_idm_train_schedule!r}."
            )
        self.noise_idm_train_schedule = schedule
        self.noise_idm_alternate_loss_scale = float(noise_idm_alternate_loss_scale)
        self.noise_idm_ddp_zero_anchor = bool(noise_idm_ddp_zero_anchor)
        self._noise_idm_micro_step = int(getattr(self, "_noise_idm_micro_step", 0))
        self._noise_idm_log_sums = getattr(self, "_noise_idm_log_sums", {"video": 0.0, "action": 0.0})
        self._noise_idm_log_counts = getattr(self, "_noise_idm_log_counts", {"video": 0, "action": 0})

    def _next_noise_idm_train_phase(self) -> str:
        if self.noise_idm_train_schedule == "joint":
            return "joint"
        micro_step = int(getattr(self, "_noise_idm_micro_step", 0))
        self._noise_idm_micro_step = micro_step + 1
        return "video" if micro_step % 2 == 0 else "action"

    def _ddp_zero_anchor(self, reference: torch.Tensor) -> torch.Tensor:
        if not self.noise_idm_ddp_zero_anchor:
    ...
    def _update_noise_idm_loss_log_average(
        self,
        name: str,
        ...
        if active:
            ...
            self._noise_idm_log_sums[name] = float(self._noise_idm_log_sums.get(name, 0.0)) + logged_value
            self._noise_idm_log_counts[name] = int(self._noise_idm_log_counts.get(name, 0)) + 1
        count = int(self._noise_idm_log_counts.get(name, 0))
        if count <= 0:
            return 0.0
        return float(self._noise_idm_log_sums[name]) / float(count)
    ...
    def _build_noise_condition_attention_mask(
        ...
        if noisy_video_tokens_per_frame != cond_video_tokens_per_frame:
            raise ValueError(
                "NoiseIDM requires identical `tokens_per_frame` for noisy and cond video branches, "
    ...
    def training_loss(self, sample, tiled: bool = False):
        if self.stack == "omnigen2":
            return self._training_loss_omnigen2_noise_idm(sample, tiled=tiled)
        ...
        if video_pre_noisy["t_mod"].ndim != 4 or video_pre_cond["t_mod"].ndim != 4:
            raise ValueError(
                "NoiseIDM requires token-wise `t_mod`; "
    ...
    def _training_loss_omnigen2_noise_idm(self, sample, tiled: bool = False):
        ...
        if train_phase == "joint":
            ...
            scale = self.noise_idm_alternate_loss_scale
        ...
        logged_loss_video = self._update_noise_idm_loss_log_average(
            name="video",
            value=loss_video,
            weight=self.loss_lambda_video,
            active=train_phase in {"joint", "video"},
        )
        logged_loss_action = self._update_noise_idm_loss_log_average(
            name="action",
            value=loss_action,
        ...
        return loss_total, {
            ...
            "noise_idm_train_phase": {"joint": 0.0, "video": 1.0, "action": 2.0}[train_phase],
            "noise_idm_loss_scale": scale,
        }
    ...
    def infer_action_omnigen2(
        ...
        if self.proprio_encoder is not None or proprio is not None:
            text_hidden, text_mask = self._append_proprio_to_context_if_enabled(
                ...
                proprio=proprio,
                source="OmniGen2 NoiseIDM action inference",
            )
        input_image = input_image.to(device=self.device, dtype=self.torch_dtype)
        ...
        self._configure_action_attention_capture(
            ...
            metadata={
                ...
                "source": "omnigen2_noise_idm_infer_action",
